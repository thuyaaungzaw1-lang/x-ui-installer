...
configure_xui() {
    ...
    # Create config file or modify existing one
    # Note: X-UI will apply these settings on first start
    cat > /usr/local/x-ui/config.yaml << EOF
# X-UI Configuration
# Generated by custom installer
server:
  port: $PORT
  username: "$USERNAME"
  password: "$PASSWORD"
EOF
    ...
    success "X-UI configured successfully"
}
...
Security Notes:
   1. Your custom credentials are set
   2. Consider using SSL certificate
